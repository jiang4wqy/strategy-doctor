...
多策略规格已经确认。C 的公共执行引擎和 adapter-driven 处方已通过 PR #8
进入 `main`。B 分支已完成增强 RSI + Bollinger + 趋势过滤器，并同步最新
`main`；当前只差合入 PR #7，随后由 A 完成全局注册和 CLI 接线。
...
剩余顺序是：合入 B PR #7、由 A 完成全局注册和 CLI 接线，最后由 D 完成
双策略示例与端到端验收。
...
- A 基础收口 PR #6 已合入。
- C 公共执行/处方 PR #8 已合入。
- 下一步合入 B PR #7；B 不直接修改 registry。
- B 合入后，A 负责 adapter 注册、CLI 接线、MA golden 复核和共享类型收口。
...
- 离线 demo 通过。
- shared engine、adapter-driven prescription 和 RSI/Bollinger focused
  tests 均通过。
