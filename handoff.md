...
多策略规格已经确认，公共契约与 adapter registry 基线已经完成。B 分支现已
实现增强 RSI + Bollinger + 趋势过滤器，但尚未注册进全局 registry，也尚未接入
公共执行和处方流程。`main` 仍是唯一公共基线。
...
公共执行引擎、通用处方流程、全局注册、CLI 接线和双策略验收尚未完成，应由
C、A、D 按固定顺序通过 PR 合入 `main`。
...
- `457e77d feat: extend mean reversion contract`
- `683f95e feat: add deterministic strategy indicators`
- `dfa8fc2 feat: add trend-filtered mean reversion adapter`
- `904dd65 test: prove mean reversion registry compatibility`
- Draft PR：`https://github.com/jiang4wqy/strategy-doctor/pull/7`
...
- 当前 A 基础收口 PR 应先于 C 合并，解除 MA adapter 对旧 prescription helper
  的反向依赖。
- 等待 C 提交公共执行引擎，并让 `src/prescribe/evolve.ts` 调用 adapter
  mutation policy；迁移完成后 C 可清理旧 MA-specific helper。
- 审查 Draft PR #7 中 B 的指标与 `rsi-bollinger.ts`；B 不直接修改 registry，A 在
  C 的公共执行/处方 PR 合入后完成 adapter 注册。
...
- 离线 demo 通过。
- adapter 架构边界测试通过。
- RSI/Bollinger focused tests 与本地双 adapter registry 测试通过。
