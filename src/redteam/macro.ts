...
export function buildMacroScenario(snapshot: MacroSnapshot, seed: number): Scenario {
  ...
  return {
    ...
    dimension: 'macro',
    sourceSkill: 'macro-analyst',
    narrative: `Fed funds ${snapshot.fedFundsLowerPct.toFixed(2)}%-${snapshot.fedFundsUpperPct.toFixed(2)}%, 10Y yield ${snapshot.treasury10yPct.toFixed(2)}%, high-yield spread ${snapshot.highYieldSpreadPct.toFixed(2)}%, DXY ${snapshot.dxy.toFixed(2)}, and VIX ${snapshot.vix.toFixed(2)} map to a deterministic ${kind} stress path.`,
