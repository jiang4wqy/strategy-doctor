...
export function buildNewsScenario(snapshot: NewsSnapshot, seed: number): Scenario {
  ...
  return {
    ...
    narrative: `Across ${snapshot.items.length} frozen news items, negative headlines represent ${(signals.negativeHeadlineShare * 100).toFixed(1)}%, high-impact headlines ${(signals.highImpactShare * 100).toFixed(1)}%, and regulatory items ${(signals.regulatoryShare * 100).toFixed(1)}%; the scenario models gap risk from sudden catalysts.`,
