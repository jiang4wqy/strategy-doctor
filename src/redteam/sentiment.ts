...
export function buildSentimentScenario(snapshot: SentimentSnapshot, seed: number): Scenario {
  ...
  const reversal = longsCrowded ? 'an upside bait move followed by a fast selloff' : 'a downside bait move followed by a fast rebound';
  ...
  return {
    ...
    narrative: `${crowdedSide}, fear-greed ${snapshot.fearGreed}, and taker buy share ${(snapshot.takerBuySellRatio * 100).toFixed(1)}%. The scenario models ${reversal} to test liquidation risk in leveraged trend strategies.`,
