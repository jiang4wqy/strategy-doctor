...
import { playbookDiagnosisRouteSchema } from '../schema.ts';
...
export interface PlaybookDiagnosisRouteOptions {
...
export const registerPlaybookRoutes:
  FastifyPluginAsync<PlaybookDiagnosisRouteOptions> = async (app, options) => {
    app.post('/api/v1/playbook/diagnoses', {
      config: {
        rateLimit: {
          max: 6,
          timeWindow: '1 minute',
      ...
      schema: playbookDiagnosisRouteSchema,
    }, async (request, reply) => {
      ...
      try {
        const imported = await importPlaybookStrategy(
          body.playbook,
          options.parse,
        );
