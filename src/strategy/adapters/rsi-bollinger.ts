...
const PARAM_LABELS: Record<keyof RsiBollingerParams, string> = {
  ...
  leverage: 'Leverage',
  stopLossPct: 'Stop-loss fraction',
  positionPct: 'Position fraction',
};
...
function targetedPatch(
  ...
  if (uniqueCauses.has('liquidation')) {
    ...
    rationale.push('Liquidation failure -> reduce leverage and keep stop-loss within half of the liquidation line');
  }
  ...
  if (uniqueCauses.has('stop-loss-bleed')) {
    ...
    rationale.push(
      'Stop-loss bleed -> widen Bollinger/RSI extremes and strengthen the trend filter',
    );
  }
