...
test('CLI renders the complete offline doctor report', () => {
  ...
  assert.ok(output.includes('Strategy Doctor Report'));
  assert.ok(output.includes('Five-Dimensional Stress Coverage'));
  assert.ok(output.includes('Three Style Scores'));
  assert.ok(output.includes('Failure List'));
...
test('CLI emits a complete JSON scorecard and writes output files', () => {
  ...
  assert.ok(readFileSync(outputPath, 'utf8').includes('Five-Dimensional Stress Coverage'));
});
